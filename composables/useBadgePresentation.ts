/*
 * Profile Link Starter - useBadgePresentation.ts
 * Resolves badge icons, colors, and provider labels for link entries.
 *
...
const getBadgeMessage = (provider: string, providerExtra?: string) => {
...
const getBadgeColor = (provider: string, customColor?: string) => {
  const key = toProviderKey(provider)
  return customColor ?? providerColors[key] ?? '#8f3f26'
}

const getBadge1Color = (_provider: string, customColor?: string) =>
  customColor ?? '#6b6b6b'

export const useBadgePresentation = () => {
